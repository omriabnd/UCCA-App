        ...
        function open(page, size, vm) {
            ...
            $uibModal.open({
                ...
                controller: function ($scope, selectionHandlerService, $q) {
                    ...
                    var lastIndex = tokenDataArray[2];

                    $scope.saveRetokenization = function () {
                        var difference = lastIndex - firstIndex + 1;
                        var splittedTokens = $scope.tokenizedText.split('*');
                        var tokenIndex = DataService.tree.tokens.map(function (x) { return uccaFactory.getIndexInTask(x); }).indexOf(firstIndex);
                        ...
                        if (Math.min.apply(Math, idTokensList) > 0) {
                            ...
                            var counter = Math.min.apply(Math, idTokensList) - 1
                        }
                        ...
                        var tokenIndex2 = DataService.tree.tokens.map(function (x) { return uccaFactory.getIndexInTask(x); }).indexOf(firstIndex);
                        ...
                        if (selectedToken[0].unitTreeId == 0) {
                            ...
                            $rootScope.$broadcast("retokenization", { oldToken: uccaFactory.getEntireTokenFromAPart(selectedToken[0]), newToken: newTokenArray });
                        }
                    }
                }
